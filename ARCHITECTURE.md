...
│  ├── Gate: skip if (current_price − avg_entry) / avg_entry < +0.5%    │
...
| Property | Value |
|----------|-------|
| Algorithm | Random Forest (n_estimators=200, max_depth=6) |
| Window size | 20 bars (4h) |
| Horizon | 5 bars (4h) |
| Sell threshold | 0.5% |
| Decision threshold | 0.597 |
| Precision / Recall / F1 | 41.3% / 99.9% / 0.585 |
...
│     c. If (current_price − avg_entry) / avg_entry < +0.5%: skip │
...
| ML SELL gate | P&L ≥ +0.5% required — computed as `(current_price − avg_entry_price) / avg_entry_price` |
...
P&L is computed directly as `(current_price − avg_entry_price) / avg_entry_price` using the same live price already fetched for inference and `position.avg_entry_price` from Alpaca. This avoids reliance on `position.unrealized_plpc`, which can diverge from the true mark-to-entry P&L in paper trading accounts due to pricing discrepancies — causing the gate to pass when it should block.
